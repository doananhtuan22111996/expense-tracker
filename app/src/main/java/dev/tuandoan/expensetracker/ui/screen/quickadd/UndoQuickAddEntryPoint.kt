 ...
 * Activities/Fragments can, and even if they could, the receiver's
 * lifecycle is too short for a scoped component to buy anything over
 * a Singleton-scope pull. This entry point pulls the five dependencies
 * the Undo flow needs directly off the application graph.
 *
 * Process-death-safe: `fromApplication` resolves the `Hilt_ApplicationComponent`
 * whether or not the app process was cold-started by the broadcast —
 * which is exactly the case we have to worry about (user taps Undo
 * from the shade after the app was memory-killed).
 *
