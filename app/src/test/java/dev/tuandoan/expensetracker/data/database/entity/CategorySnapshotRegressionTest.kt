...
class CategorySnapshotRegressionTest {
    ...
    fun categoryColorEncoding_isPinnedForSnapshots() {
        ...
        assertEquals("Available colors must match historical snapshot keys to prevent ADR-001 revert failures", expectedKeys, actualKeys)
    }
    ...
    fun categoryIconEncoding_isPinnedForSnapshots() {
        ...
        val historicalIcons = listOf(
            "restaurant", "payments", "directions_bus", "place",
            "shopping_cart", "receipt", "medical_services", "sports_esports", "school",
            "help_outline", "more_horiz"
        )
