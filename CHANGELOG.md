...
- Vietnamese (vi-VN) translation for the Summary-tab Insights section — `values-vi/strings.xml` covers all 20 Insights keys (section title, collapse toggle, biggest mover, daily pace, no-budget fallback, day-of-month, empty, error, delta chip, accessibility). Percent values (delta chip, biggest-mover headline, day-of-month headline) now render via `NumberFormat.getPercentInstance(locale)` instead of hardcoded `%%`, keeping the percent glyph positioning locale-correct. Empty-state combined copy uses a locale-format string (`insights_empty_combined`) rather than in-Kotlin concatenation, so translators can swap the em-dash separator per locale. 4 unit tests pin the `formatPercent` contract at en-US, vi-VN, zero, and 100% edges. **Inactive until a future "full app vi-VN coverage" PR re-adds `locales_config.xml` + `android:localeConfig`** — shipping the picker alongside partial translation would leave users on a mostly-English app labelled "Tiếng Việt".
