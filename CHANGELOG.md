...
- Vietnamese (vi-VN) localization for the Summary-tab Insights section — `values-vi/strings.xml` covers all 19 Insights keys (section title, collapse toggle, biggest mover, daily pace, no-budget fallback, day-of-month, empty, error, delta chip, accessibility). `res/xml/locales_config.xml` + `android:localeConfig` on `<application>` declare `en-US` and `vi-VN` so Android 13+ per-app language Settings exposes the choice. Percent values (delta chip, biggest-mover headline, day-of-month headline) now render via `NumberFormat.getPercentInstance(locale)` instead of hardcoded `%%`, keeping the percent glyph positioning locale-correct. 4 unit tests pin the `formatPercent` contract at en-US, vi-VN, zero, and 100% edges.
