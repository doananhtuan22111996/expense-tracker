...
- 10 new consent strings (`consent_prompt_v2_*`, `consent_prompt_crash_*`, `consent_prompt_analytics_*`, `consent_prompt_footer`, `consent_prompt_done`, `consent_prompt_analytics_only_*`) per Design doc's en-US copy draft. Existing v1 strings (`consent_prompt_title/body/yes/no` from PR #105) kept in place until Task 5.6 rewires `HomeScreen` — removed in v3.12.
